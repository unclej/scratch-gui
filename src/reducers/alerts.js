...
const CLOSE_STANDART_ALERT = 'scratch-gui/alerts/CLOSE_STANDART_ALERT';
...
const reducer = function (state, action) {
    ...
    case CLOSE_STANDART_ALERT: {
        const alertId = action.alertId;
        if (alertId) {
            let newList = state.alertsList.slice();
            newList = newList.filter(curAlert => alertId !== curAlert.alertId);
            return Object.assign({}, state, {
                alertsList: newList
            });
        }
...
/**
 * Function to close an alert with the given alertId.
 *
 * @param {string} alertId - id string of the alert to show
 * @return {object} - an object to be passed to the reducer.
 */
const closeStandardAlert = function (alertId) {
    return {
        type: CLOSE_STANDART_ALERT,
        alertId
    };
};
export {
